...
template <typename T>
void buildCodeTable(const std::shared_ptr<HuffmanNode<T>> &node, std::map<T, std::pair<uint32_t, int>> &table, uint32_t code = 0, int length = 0);
...
template <typename T>
std::vector<uint8_t> huffmanZstdCompress(const std::vector<T> &data,
                                         std::map<T, std::pair<uint32_t, int>> &codeTableOut,
                                         int zstdLevel = 3);

template <typename T>
std::vector<T> huffmanZstdDecompress(const std::vector<uint8_t> &compressed,
                                     const std::map<T, std::pair<uint32_t, int>> &codeTable,
                                     size_t numElements,
                                     size_t originalBitstreamSize);
...
template <typename T>
void buildCodeTable(const std::shared_ptr<HuffmanNode<T>> &node, std::map<T, std::pair<uint32_t, int>> &table, uint32_t code, int length)
{
    if (!node)
        return;
    if (node->is_leaf())
...
template <typename T>
std::vector<uint8_t> huffmanZstdCompress(const std::vector<T> &data, std::map<T, std::pair<uint32_t, int>> &codeTableOut, int zstdLevel)
{
    // Frequency count
